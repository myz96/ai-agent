...
def extract_profile_information(name):
...
def retrieve_relevant_value_prop(profileinfojson):
    ...
    print(f"Retrieving relevant value propositions...")
    time.sleep(3)
    return EXAMPLE_VALUE_PROP

def retrieve_email_thread_content(email_thread_id):
...
def get_email_thread_id(profileinfojson):
...
def compose_outreach_email(profileinfojson, relevant_value_prop, email_thread_id, email_thread_content):
